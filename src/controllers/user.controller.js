...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    {
      // we have to lookup to channel to find its subscribers
      // (Subscription) is written as (subscriptions) in database
      $lookup: {
        from: "subscriptions",
        localField: "_id",
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        // to get it as a object from mongoose it returns as a string
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [
          {
            // now we are in videos and looking into users
            $lookup: {
              from: "users",
              localField: "owner",
              foreignField: "_id",
